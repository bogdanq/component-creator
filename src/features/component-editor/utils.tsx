...
import { Attribute, Dimensions, ElementTypes, Elements, Tree } from "./models";
import { nanoid } from "nanoid";
import { defaultComponentStyles, resetStyles } from "./global-styles";
...
const styleBuilder = (attribute: Attribute, type: Elements) => {
...
export const createHtmlFromTree = (tree: Tree) => {
  const element = tree.elements.map((element) => {
    const { type, id, content, meta } = element;
    ...
    return (
      <div key={id} className={`${type}-wrapper`} id={`link_${id}`}>
        {elementByType}
      </div>
    );
  });
...
export const createStatickHTMLfromTree = (
  ...
  const result = `
    <div data-rid=${params?.rId} data-id=${params?.id} data-type=${params?.type} id=link${params?.id}>
      <style>${resetStyles.styles}</style>
      <style>${defaultComponentStyles.styles}</style>
      <style>${styles}</style>
